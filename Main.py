from Huffman import HuffmanUnzip, HuffmanZip
from LZW import LZWUnzip, LZWZip
from Zip import Zip
...
def zipLocalPath(fileName, memoryLen, lzwByteWordLen, runHuffman = True):
        '''
        helper function to zip the file at the path: '.\files\ZipedFiles'
        This function won't upload the file to a SFTP server.
        fileName: name of the file at the main path
        memoryLen: memory limit (bytes)
        lzwByteWordLen: length to use to enconde the words using LZW
        runHuffman: if True the Huffman code will be used, if Falsem the LZW code will be used
        ...
        if runHuffman:
            print("===================== Huffman =====================")
            hm = HuffmanZip(textOriginal, memoryLen, pathZipedFiles)
            hm.zip()
        else:
            print("======================= LZW =======================")
            print(f"Word Len: {lzwByteWordLen}")
            lzw = LZWZip(textOriginal, memoryLen, pathZipedFiles)
            lzw.zip(lzwByteWordLen)

def unzipLocalPath(fileName, memoryLen, lzwByteWordLen, runHuffman = True):
        '''
        helper function to unzip the file at the path: '.\\files\\UnzipedFiles'
        This function won't download the file from a SFTP server.
        fileName: name of the file to save at the main path
        memoryLen: memory limit (bytes)
        lzwByteWordLen: length that was used to enconde the words using LZW
        runHuffman: if True the Huffman code will be used, if Falsem the LZW code will be used
        '''
        pathZipedFiles = r".\files\ZipedFiles"
        savePath = r".\files\UnzipedFiles" + fileName
        ...
        if runHuffman:
            ...
            hm = HuffmanUnzip(pathZipedFiles, memoryLen, savePath)
            hm.unzip()
        else:
            print("======================= LZW =======================")
            print(f"Word Len: {lzwByteWordLen}")
            lzw = LZWUnzip(pathZipedFiles, memoryLen, savePath)
            lzw.unzip(lzwByteWordLen)

'''
You can use runStats to print all statistics or
use zip and unzip to just upload or download the files.
Use zipLocalPath to zip the file and don't upload (if you don't have the SFTP server), same to unzipLocalPath
'''
texts = [
    ...
    (r"\Cormen65MB.pdf", 13200000, 2, True, True),
    (r"\Cormen65MB.pdf", 13200000, 3, False, True),
    (r"\Cormen65MB.pdf", 13200000, 4, False, True),

    (r"\txt1GB.txt", 205000000, 2, True, True),
    (r"\txt1GB.txt", 205000000, 3, False, True),
    (r"\txt1GB.txt", 205000000, 4, False, True),
...
zipLocalPath(r"\txt100KB.txt", 210000, 2, True)
unzipLocalPath(r"\txt100KB.txt", 210000, 2, True)
